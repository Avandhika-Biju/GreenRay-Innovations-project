...
<html lang="en" >
  ...
  //Email Code End
		 ...
		 <?php
		 			$sel="select max(booking_id) as id from tbl_booking where user_id='".$_SESSION["uid"]."'";
					$dt=$con->query($sel);
					$rows=$dt->fetch_assoc();
					$up="update tbl_booking set payment_status=1, booking_date=curdate() where booking_id='".$rows["id"]."'";
					if($con->query($up))
					{
